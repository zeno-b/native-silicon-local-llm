...
class Agent:
    ...
    async def route(self, message: str, history: list[dict] | None = None) -> dict:
        ...
        options = ['{"action":"answer"} if you can answer well from your own '
                   'knowledge (reasoning, math, coding, writing, or a definition '
                   'you know).']
        if has_search:
    ...
    async def run(
        ...
        # weather, and extracts the query or the place and day from free text.
            ...
            if action == "weather" and self.registry.get("weather") is not None:
                ...
                # fetching a page if the snippet lacks the detail.
                async for event in run_and_seed(
                    "web_search",
                    {"query": decision["query"]},
                    "I searched the web to make sure this is current.",
                    ("Answer my original question using these results and cite the "
                     "URLs. If the snippets lack the detail needed, call fetch_url "
                     "on the most relevant URL. Do not repeat the same search."),
                ):
                    if "__seeded__" not in event:
                        yield event
...
# Weather intent, detected by scanning rather than by anchoring a whole
# sentence. Users phrase this every possible way ("whats tomorrows weather in
# brussels like?", "will it rain in Paris", "Tokyo forecast"), so a rigid
# template loses. Trigger on a weather word, then pull the day and place out
# wherever they sit.
# When the message is plainly about code or mechanics rather than the sky, the
# word "weather" is a red herring ("how does weather routing work in code").
WEATHER_NEGATIVE = re.compile(
    r"\b(code|routing|router|function|regex|implement|implementation|bug|api|"
    r"module|parser|pipeline|handler|endpoint|config|deploy|schema|"
    r"how does|how do i|how to)\b",
    re.I,
)
WEATHER_TRIGGER = re.compile(
    r"\b(weather|forecast|temperature|how (?:hot|cold|warm)|"
    r"(?:will|going to|gonna) (?:it )?rain|rain(?:ing)? (?:today|tomorrow|tonight))\b",
    re.I,
)
_WEEKDAYS = "monday|tuesday|wednesday|thursday|friday|saturday|sunday"
WEATHER_WHEN = re.compile(
    r"\b(today|tonight|tomorrow|" + _WEEKDAYS + r")(?:'?s)?\b", re.I,
)
# "in/at/for PLACE", stopping at a day word, trailing filler, or punctuation.
WEATHER_LOC_PREP = re.compile(
    r"\b(?:in|at|for)\s+(?P<loc>[A-Za-z][A-Za-z .'\-]*?)"
    r"(?=\s+(?:on|this|next|today|tonight|tomorrow|" + _WEEKDAYS + r"|like|please|now|right now)\b"
    r"|[?.!,]|$)",
    re.I,
)
# "PLACE weather" / "PLACE forecast".
WEATHER_LOC_SUFFIX = re.compile(
    r"\b(?P<loc>[A-Za-z][A-Za-z .'\-]*?)\s+(?:weather|forecast)\b", re.I,
)
# Tokens that are never a place, used to reject a bad suffix capture.
_NOT_A_PLACE = {
    "the", "a", "an", "todays", "today", "tonights", "tonight", "tomorrows",
    "tomorrow", "whats", "what", "hows", "how", "this", "that", "current",
    "latest", "local", "my", "our",
}


def _normalise_when(word: str) -> str:
    w = word.lower().rstrip("s").rstrip("'")
    return w if w else "today"


def weather_intent(text: str) -> tuple[str, str] | None:
    """Return (location, when) for a weather request, else None."""
    if not WEATHER_TRIGGER.search(text) or WEATHER_NEGATIVE.search(text):
        return None
    when = "today"
    m = WEATHER_WHEN.search(text)
    if m:
        when = _normalise_when(m.group(1))
    location = ""
    m = WEATHER_LOC_PREP.search(text)
    if m:
        location = m.group("loc").strip(" .'-")
        location = re.sub(r"\s+(?:on|this|next)$", "", location, flags=re.I).strip()
    if not location:
        m = WEATHER_LOC_SUFFIX.search(text)
        if m:
            candidate = m.group("loc").strip(" .'-")
            # Keep only the last word or two: "whats tomorrows brussels" should
            # not become the place. Drop leading non-place tokens.
            words = [w for w in candidate.split() if w.lower() not in _NOT_A_PLACE]
...
# Bare follow-up commands that carry no object of their own ("look it up",
# "search that", "do it"). They mean: act on what I just said. The router
# resolves them against the previous user turn.
BARE_LOOKUP = re.compile(
    r"^\s*(?:(?:can|could|would|will) you\s+)?(?:please\s+)?"
    r"(?:look\s+(?:it|that|them|this)\s+up|search(?:\s+for)?\s+(?:it|that|them|this)|"
    r"find\s+(?:it|them|that|this)(?:\s+out)?|google\s+(?:it|that|them|this)|"
    r"do\s+(?:it|that))\s*[.!]*$",
    re.I,
)
...
# Messages not worth a triage round trip: greetings, thanks, acknowledgements,
# and anything too short to be a real question. These go straight to a plain
# reply; everything else is eligible for knowledge triage.
TRIVIAL_MESSAGE = re.compile(
    r"^\s*(?:hi|hey|hello|yo|sup)(?:\s+(?:there|all|everyone|folks|claude))?"
    r"|^\s*(?:thanks|thank you|thx|ok|okay|cool|nice|got it|great|perfect|"
    r"lol|haha|bye|goodbye|good (?:morning|evening|night)|"
...
def _is_trivial(text: str) -> bool:
    m = TRIVIAL_MESSAGE.match(text)
    return bool(m and _TRIVIAL_TAIL.fullmatch(text[m.end():]))


def is_substantive(message: str) -> bool:
    """True if a message is a real question or request worth triaging."""
    text = (message or "").strip()
    if len(text) < 8 or _is_trivial(text):
        return False
    return True
...
def _last_user_turn(history: list[dict] | None) -> str | None:
    """The most recent user message in a history list, for follow-up resolution."""
    for turn in reversed(history or []):
        if turn.get("role") == "user" and (turn.get("content") or "").strip():
            return turn["content"]
    return None
...
def fast_path_call(message: str, prev_user: str | None = None) -> tuple[str, dict] | None:
    ...
    # "look it up" / "do it" refer to the previous request. Resolve them against
    # it so a follow-up does not fall through to the model, which on a small
    # model means a refusal instead of the search the user plainly wanted.
    if prev_user and BARE_LOOKUP.match(text):
        resolved = fast_path_call(prev_user)
        if resolved:
            return resolved
        prev = (prev_user or "").strip()
        # Only fall back to searching the previous turn when it was not a plain
        # definitional question the model already answered.
        prev_definitional = DEFINITIONAL.match(prev) and not STRONG_RECENCY.search(prev)
        if 2 <= len(prev) <= 200 and not prev_definitional:
            return "web_search", {"query": clean_search_query(prev)}
        return None
...
class Agent:
    ...
    async def needs_search(self, question: str) -> bool:
        """One-word knowledge assessment: does answering this need a lookup?

        A 3B is poor at deciding to emit a tool call unprompted but decent at a
        constrained yes/no when that is the only thing asked of it. The result
        gates a deterministic search seed, so a mistaken ANSWER still leaves the
        normal loop free to call a tool, and a mistaken SEARCH costs one query.
        """
        if self.registry.get("web_search") is None:
            return False
        prompt = [
            {"role": "system", "content":
                "You are a router. Decide whether answering the user's message "
                "correctly needs a web search. Reply with exactly one word.\n"
                "SEARCH if it depends on current events, real-time data (weather, "
                "prices, scores), recent releases or version numbers, specific "
                "facts that may have changed, named entities you are unsure about, "
                "or anything after your training. Also SEARCH if you are not "
                "confident you know the answer accurately.\n"
                "ANSWER if it is general knowledge, reasoning, math, coding, "
                "writing, or a definition you know well.\n"
                "When in doubt, reply SEARCH."},
        ...
        try:
            ...
            log(f"Knowledge triage failed ({exc}); defaulting to no search.", logging.WARNING)
            return False
        verdict = strip_reasoning(text).strip().upper()
        # Bias toward search: treat anything that is not a clear ANSWER as SEARCH,
        # but require the word to actually appear so an empty reply does not
        # trigger a pointless query.
        if "ANSWER" in verdict and "SEARCH" not in verdict:
            return False
        return "SEARCH" in verdict
    ...
    async def run(
        ...
        if self.config.fast_path:
            shortcut = fast_path_call(user_message, _last_user_turn(history))
            if shortcut and self.registry.get(shortcut[0]) is not None:
                name, args = shortcut
                yield {"type": "tool_call", "name": name, "args": args, "step": 0,
                       "fast_path": True}
        ...
        # No deterministic shortcut fired and nothing is seeded yet. Fall back to
        # model-judged knowledge triage: ask the model whether the question needs
        # a lookup, and if so seed the search the same way the fast path does.
        # This is the general form of the fast-path regexes: it covers phrasings
        # no pattern anticipates, and it is what makes the model "assess if the
        # answer is within its knowledge, look up online if not".
        if not scratch and self.config.knowledge_triage and is_substantive(user_message):
            if await self.needs_search(user_message):
                query = clean_search_query(user_message) or user_message[:200]
                yield {"type": "tool_call", "name": "web_search",
                       "args": {"query": query}, "step": 0, "triage": True}
                result, error = await asyncio.to_thread(
                    self.registry.call, "web_search", {"query": query}, conversation_id
                )
                yield {"type": "tool_result", "name": "web_search", "result": result,
                       "error": error, "step": 0, "triage": True}
                if not error:
                    trace.append({"name": "web_search", "args": {"query": query},
                                  "result": result[:1000], "error": None})
                    seen_calls.append(
                        "web_search:" + json.dumps({"query": query}, sort_keys=True,
                                                   ensure_ascii=False, default=str)
                    )
                    budget = self._tool_budget(reserve)
                    seeded, _ = await self.compress_tool_result("web_search", result, budget)
                    scratch.append({"role": "assistant",
                                    "content": "I searched the web to make sure this is current."})
                    scratch.append({"role": "user",
                                    "content": f"TOOL RESULT [web_search]:\n{seeded}\n\n"
                                               "Answer my original question using these results and "
                                               "cite the URLs. If the snippets lack the detail needed, "
                                               "call fetch_url on the most relevant URL. Do not repeat "
                                               "the same search."})
...
def create_app(
    ...
    async def chat(request: ChatRequest):
        ...
        max_tokens, temperature = overrides(request)
        use_agent = config.agent_enabled if request.agent is None else request.agent
        # Mirror the streaming handler: a lookup request searches even with the
        # agent toggle off. See the note there.
        if not use_agent and config.fast_path and fast_path_call(
            request.message, _last_user_turn(history)
        ):
            use_agent = True
        # Even without a regex match, a substantive question routes through the
        # agent so its knowledge-triage step can decide whether to search. Chit
        # chat and one-word replies stay on the cheap plain path below.
        elif not use_agent and config.knowledge_triage and is_substantive(request.message):
            use_agent = True
    ...
    async def chat_stream(request: ChatRequest):
        ...
        max_tokens, temperature = overrides(request)
        use_agent = config.agent_enabled if request.agent is None else request.agent
        # A lookup request must search even when the agent toggle is off, or the
        # model answers tool-less from stale weights and refuses. The fast path
        # is cheap: one search plus one synthesis, not the full multi-step loop,
        # and it only engages when fast_path_call actually matches. Normal chat
        # with the toggle off still takes the plain completion below.
        if not use_agent and config.fast_path and fast_path_call(
            request.message, _last_user_turn(history)
        ):
            use_agent = True
...
def selftest() -> int:
    ...
    for text, expected in [
        ...
        ("whats tomorrows weather in brussels like?", "weather"),
        ("how does weather routing work in code?", None),
    ...
    follow = fast_path_call("look it up", "whats tomorrows weather in brussels like?")
    if (follow[0] if follow else None) != "weather":
        failures.append(f"bare follow-up did not resolve to weather: {follow!r}")
    if fast_path_call("look it up", "what is a CVE?") is not None:
        failures.append("bare follow-up force-searched a definitional prev turn")
    for msg, want_substantive in [("hi", False), ("thanks!", False),
                                  ("what are the latest CVEs?", True),
                                  ("explain how a hash map works", True)]:
        if is_substantive(msg) != want_substantive:
            failures.append(f"is_substantive({msg!r}) != {want_substantive}")
